...
const generatePageMap = (options: SitemapOptions, app: App): Map<string, SitemapPageInfo> => {
  const {
    ...
    modifyTimeGetter = ((page: Page<{ git: GitData }>): string =>
      page.data.git?.updatedTime ? new Date(page.data.git.updatedTime).toISOString() : "") as ModifyTimeGetter
  } = options;
  ...
  pages.forEach((page) => {
    ...
    if (relatedLocales.length > 1) {
      ...
      if (app.env.isDebug)
        relatedLocales.forEach((localePrefix) => {
          if (!locales[localePrefix].lang && !reportedLocales.includes(localePrefix)) {
            logger.warn(`[@vuepress/plugin-sitemap] 'lang' option for ${localePrefix} is missing`);
      ...
      links = relatedLocales.map((localePrefix) => ({
        ...
        url: `${base.replace(/\/$/, "")}${defaultPath.replace(/^\//u, localePrefix)}`
      }));
    }
    ...
    if (app.env.isDebug) {
      logger.info(`[@vuepress/plugin-sitemap] sitemap option for ${page.path}`, sitemapInfo);
    }
...
export const generateSiteMap = async (options: SitemapOptions, app: App): Promise<void> => {
  ...
  await withSpinner(`Generating sitemap to ${chalk.cyan(sitemapFilename)}`)(
    () =>
      new Promise<void>((resolve) => {
        ...
        extraUrls.forEach((item) => sitemap.write({ url: `${base}${item.replace(/^\//u, "")}` }));
        sitemap.end(() => {
          resolve();
        });
      })
  ...
  await withSpinner(`Appended sitemap path to ${chalk.cyan("robots.txt")}`)(async () => {
